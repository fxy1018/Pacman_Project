...
#         util.raiseNotDefined()
        ...
        #check if a ghost has been captured
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            ...
            particles.append(self.legalPositions[i%numPos])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            #if all weight are zero
            if self.isAllZero(allPossible):
                self.initializeUniformly(gameState)
            else:
                samples = []
                for i in range(self.numParticles):
                    samples.append(util.sample(allPossible)) 
                self.particles = samples       
#         util.raiseNotDefined()
    
    def isAllZero(self, counter):
        for key in counter:
            if counter[key] != 0:
                return False
        return True 
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        particles = []        
        ghostPos = self.legalPositions
        permutePos = [p for p in itertools.product(ghostPos, repeat = self.numGhosts)]
        
        for i in range(self.numParticles):
            numPos = len(permutePos)
            particles.append(tuple(permutePos[i%numPos]))
